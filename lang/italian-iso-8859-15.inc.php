<?php
/* $Id: italian-iso-8859-15.inc.php,v 2.29.2.2 2005/11/29 14:15:02 lem9 Exp $ */
...
$strDrop = 'Elimina';
$strDropSelectedDatabases = 'Cancella i database selezionati';
...
$strEnclosingOptional = 'L\'inclusione dei campi è opzionale';
$strEndCut = 'FINE CUT';
$strEnd = 'Fine';
$strEndRaw = 'FINE RAW';
...
$strFileNameTemplateHelp = 'Usare __DB__ per i nomi dei database, __TABLE__ per i nomi delle tabelle e %sany strftime%s opzione per le specifiche temporali, le estensioni saranno automaticamente aggiunte . Tutto il resto del testo verrà preservato.';
...
$strInvalidFieldAddCount = 'Devi aggiungere come minimo un campo.';
...
$strShowStatusDelayed_errorsDescr = 'Numero di righe scritte con INSERT DELAYED in cui ci sono stati degli errori (probabilmente chiave duplicata).';
...
$strShowStatusHandler_discoverDescr = 'Il server MySQL può chiedere al motore di storage NDB Cluster se conosce una tabella sulla base di un nome dato. Questo è chiamato discovery. Handler_discover indica il numero di volte che una tabella è stata trovata.';
$strShowStatusHandler_read_firstDescr = 'Il numero di volte che il primo valore è stato letto da un indice. Se è troppo alto è probabile che il server stia facendo molte scansioni complete degli indici; per esempio, SELECT col1 FROM foo, assumendo che col1 sia indicizzata.';
...
$strShowStatusInnodb_buffer_pool_pages_latchedDescr = 'Il numero di pagine bloccate in un InnoDB buffer pool. Queste pagine sono attualmente in lettura o in scrittura e non possono essere aggiornate o rimosse per altre ragioni.';
...
$strShowStatusInnodb_buffer_pool_wait_freeDescr = 'Normalmente le scritture nel buffer pool InnoDB vengono effettuate in background. Tuttavia se è necessario leggere o creare una pagina, e non sono disponibile pagine pulite è necessario attendere che le pagine siano aggiornate prima. Questo contatore conta le istanze di queste attese. Se la dimensione del buffer pool è stata settata correttamente questo valore dovrebbe essere basso.';
...
$strShowStatusInnodb_page_sizeDescr = 'La dimensione di-compilazione delle pagine InnoDB (default 16KB). Molti valori sono conteggiati nelle pagine; la dimensione delle pagine permette di convertirli facilmente in bytes.';
...
$strShowStatusKey_blocks_not_flushedDescr = 'Il numero di blocchi chiave aggiunti nella cache chiave che sono stati cambiati, ma che non sono stai aggiornati su disco. E\' conosciuto con il nome di Not_flushed_key_blocks.';
...
$strShowStatusKey_blocks_usedDescr = 'Il numero di blocchi usati nella cache chiave. Questo valore è un\'importante segnale che indica il numero massimo di blocchi che sono stati in uso contemporaneamente.';
$strShowStatusKey_read_requestsDescr = 'Il numero di richieste per leggere un blocco chiave dalla cache.';
$strShowStatusKey_readsDescr = 'Il numero di letture fisiche dal disco di un blocco chiave. Se Key_reads è grande allora il valore key_buffer_size è probabilmente troppo piccolo. Il rapporto di cache miss rate può essere calcolato come Key_reads/Key_read_requests.';
...
$strShowStatusSort_merge_passesDescr = 'Il numero di passaggi di fusione che l\'algoritmo di ordinamento ha dovuto fare. Se questo valore è grande, dovresti incrementare la variabile di sistema sort_buffer_size.';
...
$strShowStatusThreads_createdDescr = 'Il numero di processi creati per gestire le connessioni. Se Threads_created è grosso, devi probabilmente aumentare il valore thread_cache_size. (Normalmente questo non fornisce un significativo incremento delle performace se hai una buona implementazione dei processi.)';
